...
The paper trading engine simulates a trading account and persists the journal to JSON so state survives restarts. 
Two filters (`is_duplicate_setup`, `is_invalidated_level`) prevent common failure modes: scanners that re-trigger 
on the same bar producing duplicates, and re-entries after a stop loss at a level that hasn't structurally 
invalidated. Metrics are computed on-demand from the journal for dashboard and analysis.

## Key Files

| File | Description |
|------|-------------|
| `trader.py` | `PaperTrader` class: opens trades via `execute(SetupResult)`, closes on TP/SL via `check_exits()`, stores account state (balance, open/closed trades) |
...
| `__init__.py` | Exports `PaperTrader`, `Trade`, `is_duplicate_setup`, `is_invalidated_level`, `calculate_metrics` |
...
- `PaperTrader` wraps a JSON journal at `journal_path` (typically `traders/<name>/trades.json`).
...
- `PaperTrader.execute(setup: SetupResult)` opens a trade: validates setup status and prices, sizes via 
  `balance * per_trade_pct / 100 / stop_distance`, applies leverage cap (`notional <= balance * max_leverage`), 
  writes to journal. Returns `Trade | None`.
- `PaperTrader.check_exits()` fetches the latest 1m candle for each open trade, checks candle high/low against 
  SL and TP. If SL hit, closes with "SL_HIT"; if TP hit, closes with "TP_HIT". Computes PnL and R-multiple. 
...
- Leverage cap: `notional = size * entry` must not exceed `balance * max_leverage`. If it would, shrinks size accordingly.
- Always computes `rr_ratio = target_distance / stop_distance`.
